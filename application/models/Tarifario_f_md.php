...
class Tarifario_f_md extends CI_Model {
	...
	function getPrecioPaquete($cliente_id, $paquete){
        ...
        //GESTION DE FLOTA
        if($esquema[0]['gestionFlota'] && $paquete['tipo_vehiculo'] && $paquete['tipo_hora']){
            $precio = $this->getPrecioPorGestionFlota($cliente_id, $esquema, $paquete, $precio);
        }
    ...
    function getPrecioPorGestionFlota($cliente_id, $esquema, $paquete, $precio){
        $error = "";
        
        $tipo_vehiculo = $paquete['tipo_vehiculo'];
        $tipo_hora = $paquete['tipo_hora'];

        $gestionFlota = $this->getGestionFlota($cliente_id, $paquete['zonaA'], $paquete['zonaB'], $tipo_vehiculo, $tipo_hora);

        foreach ($gestionFlota as $gf) {
            
            if($gf->tipo_vehiculo == $tipo_vehiculo && $gf->tipo_hora == $tipo_hora){
                $precio = bcmul($gf->precio, $paquete['cantidad_hora']);
            }
    ...
    */
    ...
    function getGestionFlota($cliente, $zonaA, $zonaB, $tipo_vehiculo, $tipo_hora){
        $query = "SELECT G.* 
                FROM flash_tarifario T
                INNER JOIN flash_clientes C ON T.cliente_id = C.id
                INNER JOIN flash_esquema E ON T.esquema_id = E.id
