    ...
    //        echo($this->db->last_query());die;
         ...
         function getDespachoPiezasAgrupadas($despacho_id){
            ...
            $query2 = 'SELECT   fp.*,
                        ...
                        LEFT JOIN flash_comprobantes_ingresos ci ON ci.id = p.comprobante_ingreso_id
                        INNER JOIN flash_piezas_estados_variables ev ON ev.id = p.estado_id
                        LEFT  JOIN users u on u.id = p.verifico_id
                        WHERE p.tipo_id = '.Pieza::TIPO_NORMAL
                        .' AND fp.despacho_id = '.$despacho_id ;
    ...
    public function getDdisPendientesLiquidar($periodo_desde, $periodo_hasta,$periodo_alta_desde, $periodo_alta_hasta, $sucursal_id){
            ...
            $subquery = "   SELECT sp2.id FROM flash_subpiezas sp2
             ...
             function getPreciosEspecialesXCliente($grupo=null, $servicio=null, $activo=null, $buscar=null, $cliente_id){
                ...
                $query = $this->db
                                ->select('p.*, c.nombre AS cliente, s.nombre AS servicio')
                                ->join('flash_clientes c', 'c.id = p.cliente_id')
        ...
        function getHistoricoPreciosEspeciales($cliente_id, $fecha_desde, $fecha_hasta, $servicio_id){
            ...
            if($servicio_id != null && $servicio_id != "-1" ) {
            ...
            $query = $this->db
                                ->select('c.nombre cliente, s.nombre servicio, ape.fecha_vto_posterior fecha_vto, 
                                            ape.precio_nuevo, DATE_FORMAT(ape.`date_create`, "%d-%m-%Y") ultima_actualizacion,
                                            u.username')
                                ->join('flash_clientes c', 'c.id = ape.cliente_precio_especial_id')
				                ->join('flash_servicios s', 's.id = ape.servicio_id')
                                ->join('users u', 'u.id = ape.user_id_create')                            
                                ->where('ape.cliente_precio_especial_id', $cliente_id)
