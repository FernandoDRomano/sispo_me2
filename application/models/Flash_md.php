...
class Flash_md extends CI_Model
{
    ...
    function getClientes($estado = null, $tipo = null, $buscar = null)
    {
        ...
        $query = $this->db
            ->select('c.*, t.nombre AS tipo, e.nombre AS estado, p.nombre as paisNombre, pr.nombre as provinciaNombre, l.nombre as localidadNombre, u.username, u.email')
            ->join('flash_clientes_tipos t', 't.id = c.tipo_cliente_id')
            ->join('flash_clientes_estados e', 'e.id = cliente_estado_id')
            ->join('flash_paises p', 'p.id = c.pais', 'left')
            ->join('ubicacion_provincias pr', 'pr.id = c.provincia', 'left')
            ->join('ubicacion_localidades l', 'l.id = c.localidad', 'left')
            ->join('users u', 'u.id = c.user_id', 'left')
            ->get('flash_clientes c');

        //$aux = $this->db->last_query();
        //print_r($aux);
        //die;
    ...
    function getDepartamentos_altapieza($cliente)
    {
        $query = $this->db
            ->select('d.*, c.nombre AS cliente')
            ->join('flash_clientes c', 'c.id = d.cliente_id')
    ...
    function getCarteros($sucursal = NULL, $buscar = NULL, $cartero_tipo_id = NULL)
    {
        ...
        $query = $this->db
            ->select('c.*, s.nombre AS sucursal, t.nombre cartero_tipo, p.nombre as pais, prov.nombre as provincia, l.nombre as localidad, iva.descripcion as condicionFiscal')
            ->join('flash_sucursales s', 's.id = c.sucursal_id')
            ->join('flash_sucursales_carteros_tipos t', 't.id = c.cartero_tipo_id', 'left')
            ->join('flash_paises p', 'p.id = c.pais')
            ->join('ubicacion_provincias prov', 'prov.id = c.provincia')
            ->join('ubicacion_localidades l', 'l.id = c.localidad')
            ->join('flash_iva iva', 'iva.id = c.condicion_fiscal')
            ->where('c.deleted_user_id IS NULL')
    ...
    function getPiezasPorServiciosPorComprobante($comprobante_id)
    {
        $user_row = $this->ion_auth->user()->row();
        $query1 = $this->db
            ->select('p.id pieza_id,p.tipo_id, p.domicilio, p.localidad_destino, p.codigo_postal_destino, svc.acuse acuse,
                                    s.*,
    ...
    function getDespachoPiezasAgrupadas($despacho_id)
    {
        ...
        $query1 = 'SELECT   fp.*,
                                ...
                                p.servicio_id,
                                COUNT(*) cantidad,
                                CONCAT((p.destinatario), (", "), (p.domicilio), (", "), ( p.codigo_postal_destino), (", "), ( p.localidad_destino)) AS descripcion,
                                ev.nombre AS estado,
    ...
    */
    ...
    public function getConsultasGlobales(
        ...
        if ($codigo_postal != '') $query = $this->db->like('p.codigo_postal_destino', $codigo_postal);
        if ($localidad != '') $query = $this->db->like('p.localidad_destino', $localidad);
        ...
        if ($datos_varios_1 != '') $query = $this->db->like('p.datos_varios_1', $datos_varios_1);
        if ($datos_varios_2 != '') $query = $this->db->like('p.datos_varios_2', $datos_varios_2);
        if ($datos_varios_3 != '') $query = $this->db->like('p.datos_varios_3', $datos_varios_3);
        ...
        $query = $this->db
            ->select(' (CASE WHEN ci.create IS NULL THEN "" ELSE DATE_FORMAT(ci.create,"%d-%m-%Y") END) AS fecha_ingreso ,
                                ...
                                (CASE WHEN p.codigo_postal_destino IS NULL THEN "" ELSE p.codigo_postal_destino END) AS codigo_postal ,
                                (CASE WHEN p.localidad_destino IS NULL THEN "" ELSE p.localidad_destino END) AS localidad ,
                                ...
                                (CASE WHEN cis.remito IS NULL THEN "" ELSE cis.remito END) AS remito')
            ->join('flash_comprobantes_ingresos ci', 'ci.id = p.comprobante_ingreso_id')
            ->join('flash_clientes c', 'c.id = ci.cliente_id')
            ->join('flash_comprobantes_ingresos_servicios cis', 'cis.id = p.servicio_id')
    ...
    public function getConsultasGlobalesClientes(
        $fecha_ingreso = '',
        $pieza_id = '',
        $barra_externa = '',
        $comprobante = '',
        ...
        if ($barra_externa != '') $query = $this->db->like('p.barcode_externo', $barra_externa);
        if ($comprobante != '') $query = $this->db->like('ci.numero', $comprobante);
        if ($cliente != '') $query = $this->db->where('c.user_id = ', $cliente);
        if ($servicio != '') $query = $this->db->like('s.nombre', $servicio);
        if ($cartero != '') $query = $this->db->like('sc.apellido_nombre', $cartero);
        if ($hoja_ruta_id != '') $query = $this->db->where('hdr.id = ', $hoja_ruta_id);
        if ($despacho_id != '') $query = $this->db->where('d.id = ', $despacho_id);
    ...
    public function getConsultasGlobalesPaginado(
        $fecha_ingreso = '',
        $pieza_id = '',
        $barra_externa = '',
        $comprobante = '',
        ...
        if ($codigo_postal != '') $query = $this->db->like('p.codigo_postal_destino', $codigo_postal); //aqui
        if ($localidad != '') $query = $this->db->like('p.localidad_destino', $localidad); //aqui
        ...
        if ($datos_varios_1 != '') $query = $this->db->like('p.datos_varios_1', $datos_varios_1); //aqui
        if ($datos_varios_2 != '') $query = $this->db->like('p.datos_varios_2', $datos_varios_2); //aqui
        if ($datos_varios_3 != '') $query = $this->db->like('p.datos_varios_3', $datos_varios_3); //aqui
        //if ($datos_varios_4 != '') $query = $this->db->like('p.datos_varios_4',$datos_varios_4 ); //aqui
    ...
    public function getConsultasGlobalesPaginadoCliente(
        $fecha_ingreso = '',
        $pieza_id = '',
        $barra_externa = '',
        $comprobante = '',
        ...
        if ($comprobante != '') $query = $this->db->like('ci.numero', $comprobante);
        //if ($cliente != '') $query = $this->db->like('c.nombre',$cliente );
        //PARA FILTRAR POR EL ID DEL USUARIO ASOCIADO AL CLIENTE
        if ($cliente != '') $query = $this->db->where('c.user_id = ', $cliente);
        ...
        $query = $this->db
            ->select(' (CASE WHEN ci.create IS NULL THEN "" ELSE DATE_FORMAT(ci.create,"%d-%m-%Y") END) AS fecha_ingreso ,
                            (CASE WHEN p.id IS NULL THEN "" ELSE p.id END) AS pieza_id ,
                            (CASE WHEN pp.descripcion_paquete IS NULL THEN "" ELSE pp.descripcion_paquete END) AS descripcion_paquete , #agregado
                            (CASE WHEN p.barcode_externo IS NULL OR p.barcode_externo = "" THEN "" ELSE p.barcode_externo END) AS barcode_externo ,
                            (CASE WHEN ci.numero IS NULL THEN "" ELSE ci.numero END) AS comprobante ,
                            (CASE WHEN c.nombre IS NULL THEN "" ELSE c.nombre END) AS cliente ,
                            (CASE WHEN s.nombre IS NULL THEN "" ELSE s.nombre END) AS servicio ,
                            ...
                            (CASE WHEN p.celular_dest IS NULL THEN "" ELSE p.celular_dest END) AS celular_dest ,
                            (CASE WHEN cis.remito IS NULL THEN "" ELSE cis.remito END) AS remito ')
            ->join('flash_piezas_paquetes pp', 'pp.pieza_id = p.id') //agregado
            ->join('flash_comprobantes_ingresos ci', 'ci.id = p.comprobante_ingreso_id')
            ->join('flash_clientes c', 'c.id = ci.cliente_id')
            ->join('flash_comprobantes_ingresos_servicios cis', 'cis.id = p.servicio_id')
            ->join('flash_servicios s', 'cis.servicio_id = s.id')
    ...
    /*
        public function getConsultasGlobalesTotal($fecha_ingreso,$pieza_id, $barra_externa, $comprobante,$cliente, $servicio,
                ...
                if ($datos_varios_2 != '') $query = $this->db->like('p.datos_varios_2',$datos_varios_2 );
                if ($datos_varios_3 != '') $query = $this->db->like('p.datos_varios_3',$datos_varios_3 );
                //if ($datos_varios_4 != '') $query = $this->db->like('p.datos_varios_4',$datos_varios_4 );
    ...
    public function getConsultasGlobalesTotal(
        $fecha_ingreso,
        $pieza_id,
        $barra_externa,
        $comprobante,
        ...
        /* Solucion temporal solo traer los 2 ultomos meses */
        $query = $this->db->where('ci.create >= date_sub(curdate(), interval 12 month)');
        /* END: Solucion temporal solo traer los 2 ultomos meses */
    ...
    public function getConsultasGlobalesTotalCliente(
        $fecha_ingreso,
        $pieza_id,
        $barra_externa,
        $comprobante,
        ...
        if ($comprobante != '') $query = $this->db->like('ci.numero', $comprobante);
        //if ($cliente != '') $query = $this->db->like('c.nombre', $cliente); 
        //PARA FILTRAR POR EL USUARIO DEL CLIENTE
        if ($cliente != '') $query = $this->db->where('c.user_id = ', $cliente);
    ...
    function getPiezasPorServiciosPorComprobanteSinGroup($comprobante_id, $servicio_id, $limite = NULL)
    {
        ...
        $query1 = $this->db
            ->select('cis.*,
                                    ...
                                    p.codigo_postal_destino codigo_postal,
    ...
    function getLiquidacionesCarteros($liquidacion_cartero_id)
    {
        ...
        //Agregue para que el usuario Habidc pueda ver la liquidacion de todas las sucursales. 
        //if (!$this->ion_auth->in_group(array(1, 2), $user_row->id)) $query = $this->db->where('sucursal_id', $user_row->sucursal_id);
        if (!$this->ion_auth->in_group(array(1, 2), $user_row->id) && $user_row->id !=  408) $query = $this->db->where('sucursal_id', $user_row->sucursal_id);
    ...
    function getListaHojasRutas($sucursal_id, $limit = 25, $start = 0, $hdr_id = null)
    {
        $user_row = $this->ion_auth->user()->row();
        //agregue el grupo 28 
        if (!$this->ion_auth->in_group(array(1, 2, 28), $user_row->id)) $query = $this->db->where('hdr.sucursal_id', $user_row->sucursal_id);
    ...
    public function getPiezasSimplesNovedadesPorComprobante($estado_id, $comprobante_nro, $servicio_id, $cantidad, $limite_inferior)
    {
        $user_row = $this->ion_auth->user()->row();
        $is_user = $this->ion_auth->in_group(array(3), $user_row->id);
        /*if(!$this->ion_auth->is_admin($user_row->id))*/
        $query = $this->db->where('p.sucursal_id', $user_row->sucursal_id);
    ...
    public function grabarDetalleRendicion(
        $cliente_id,
        $departamento_id,
        $sucursal_id,
        $servicio_id,
        ...
        if ($busqueda != null) {
            $where .= " AND   (CONVERT(p.id,CHAR) LIKE '%" . $busqueda . "%' ESCAPE '!' 
                                ...
                                OR  p.codigo_postal_destino LIKE '%" . $busqueda . "%' ESCAPE '!'
    ...
    function getPiezasPorHDRGroupSimples($hoja_ruta_id)
    {
        $user_row = $this->ion_auth->user()->row();
        $query1 = $this->db
            ->select(' c.nombre cliente, "" as cantidad, sp.pieza_id pieza_id, p.tipo_id, pt.nombre tipo , p.servicio_id servicio_id, s.nombre servicio, ev.nombre estado, p.barcode, p.destinatario, p.domicilio, p.localidad_destino, p.codigo_postal_destino ')
            ->join('flash_subpiezas sp', 'hdr.id = sp.hoja_ruta_id')
            ->join('flash_piezas p', 'sp.pieza_id = p.id')
            ->join('flash_comprobantes_ingresos_servicios cis', 'p.servicio_id = cis.id')
            ->join('flash_servicios s', 'cis.servicio_id = s.id')
        ...
        $join1 = $this->db->last_query();
        $query2 = $this->db
            ->select(' c.nombre cliente, count(*) cantidad, sp.pieza_id pieza_id, p.tipo_id, pt.nombre tipo , p.servicio_id servicio_id, s.nombre servicio, ev.nombre estado, p.barcode, p.destinatario, p.domicilio, p.localidad_destino, p.codigo_postal_destino ')
            ->join('flash_subpiezas sp', 'hdr.id = sp.hoja_ruta_id')
            ->join('flash_piezas p', 'sp.pieza_id = p.id')
            ->join('flash_comprobantes_ingresos_servicios cis', 'p.servicio_id = cis.id')
            ->join('flash_servicios s', 'cis.servicio_id = s.id')
    ...
    public function getHDRPendientesXClientes($periodo_desde, $periodo_hasta, $cliente_id, $sucursal_id)
    {
        ...
        $query = $this->db
            ->select('suc.nombre sucursal, ci.cantidad cantidad,DATE_FORMAT(ci.create,"%d-%m-%Y") fecha_ingreso,
                                        ...
                                        p.domicilio, p.localidad_destino, p.barcode_externo codigo_externo
                                        ')
            ->join('flash_comprobantes_ingresos ci', 'p.comprobante_ingreso_id = ci.id')
            ->join('flash_comprobantes_ingresos_servicios cis', 'p.servicio_id = cis.id')
            ->join('flash_servicios s', 'cis.servicio_id = s.id')
    ...
    function getliquidacionesDdisResumen($liquidacion_cartero_id)
    { //Quinta pestaña
        $user_row = $this->ion_auth->user()->row();
        //if (!$this->ion_auth->in_group(array(1, 2), $user_row->id)) $query = $this->db->where('sc.sucursal_id', $user_row->sucursal_id);
    ...
    function getliquidacionesDdisResumenClientes($liquidacion_ddi_id)
    { //Quinta pestaña
        $user_row = $this->ion_auth->user()->row();
        //if (!$this->ion_auth->in_group(array(1, 2), $user_row->id)) $query = $this->db->where('sc.sucursal_id', $user_row->sucursal_id);
        $query = $this->db
    ...
    function getPreciosEspecialesXCliente($grupo = null, $servicio = null, $activo = null, $buscar = null, $cliente_id)
    {
        if ($grupo != null && $grupo != 0) {
            if ($grupo != null && $grupo != 0) $query = $this->db->where('p.grupo_servicio_id', $grupo);
            if ($servicio != null && $servicio != 0) $query = $this->db->where('p.servicio_id', $servicio);
        ...
        $query = $this->db
            ->select('p.*, c.nombre AS cliente, s.nombre AS servicio, g.nombre grupo')
            ->join('flash_clientes c', 'c.id = p.cliente_id')
            ->join('flash_servicios_grupos g', 'g.id = p.grupo_servicio_id', 'left')
            ->join('flash_servicios s', 's.id = p.servicio_id')
    ...
    function getHistoricoPreciosEspeciales($cliente_id, $fecha_desde, $fecha_hasta, $servicio_id)
    {

        if ($servicio_id != null && $servicio_id > 0) {
            $query = $this->db->where('s.id', $servicio_id);
        }
        if ($cliente_id != null && $cliente_id > 0) {
            $query = $this->db->where('cpe.cliente_id', $cliente_id);
        }
        $fecha_desde = new DateTime($fecha_desde);
        $fecha_hasta = new DateTime($fecha_hasta);

        $query = $this->db
            ->select('ape.id historico_id, c.nombre cliente, s.nombre servicio, DATE_FORMAT(ape.fecha_vto_anterior, "%d-%m-%Y") fecha_vto, ape.precio_anterior,
                                            ape.precio_nuevo, DATE_FORMAT(ape.`date_create`, "%d-%m-%Y") ultima_actualizacion,
                                            u.username')
            ->join('flash_clientes_precios_especiales cpe', 'cpe.id = ape.cliente_precio_especial_id')
            ->join('flash_clientes c', 'c.id = cpe.cliente_id')
            ->join('flash_servicios s', 's.id = ape.servicio_id')
            ->join('users u', 'u.id = ape.user_id_create')
            ->where('ape.date_create >= ' . "'" . $fecha_desde->format('Y-m-d 00:00:00') . "'")
            ->where('ape.date_create <= ' . "'" . $fecha_hasta->format('Y-m-d 23:59:59') . "'")
            ->order_by('actualizacion_id', 'desc')
            ->get('flash_actualizacion_precios_especiales ape');
        //echo $this->db->last_query();die;
        $result = $query->result();
        return $result;
    }
    ...
    {
        $user_row = $this->ion_auth->user()->row();
        if ($this->ion_auth->in_group(array(1, 2, 28), $user_row->id) or $user_row->id == 224 or $user_row->id == 195) {
            return Sucursal::all();
        } else {
            return Sucursal::whereId($user_row->sucursal_id)->get();
        }
    }

    public function isAllSucursales()
    {
        $user_row = $this->ion_auth->user()->row();
        return ($this->ion_auth->in_group(array(1, 2, 28), $user_row->id) or $user_row->id == 224 or $user_row->id == 195);
    }
}
