...
function dashboardCargarMensajeComerciales(){
    var url = '<?php echo base_url()."backend/dashboard/mensaje_comerciales"?>';
    $.ajax({
        type: "POST",
        url: url,
        data: {},
        cache: false,
        beforeSend: function(){
            $('#dashboard_mensaje_comerciales').html('<div style="padding-top:110px;" class="text-center"><h2><i class="fa fa-refresh fa-spin"></i></h2></div>');
        },
        success: function(respuesta){
            $('#dashboard_mensaje_comerciales').html(respuesta);
        }
    });
}
...
$(document).ready(function(){
    ...
    //dashboardCargarMensajeComerciales();
});
</SCRIPT>