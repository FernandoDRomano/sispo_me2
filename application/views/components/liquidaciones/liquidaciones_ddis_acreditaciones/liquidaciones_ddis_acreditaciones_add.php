            ...
            <tbody id="body-grilla">
                <?php foreach ($results as $result) { ?>
                    <tr id="resultados" class="nueva">
                        ...
                        <td>
                            <div class="btn-group">
                              <?php if($permisos_efectivos->update==1) { ?><a href="<?php echo base_url().'liquidaciones/liquidaciones_ddis_acreditaciones/autorizarAcreditacionDdi/'.$result->acreditacion_id ?>" <?php echo $result->fecha_autorizacion!=NULL?'class="btn btn-primary"':'class="btn btn-danger"'?>><i class="fa fa-check-square-o"></i></a><?php } ?>
                              <?php if($permisos_efectivos->update==1) { ?><a data-toggle="modal" href="<?php echo base_url().'liquidaciones/liquidaciones_ddis_acreditaciones/edit/'.$result->acreditacion_id?>" data-target="#myModal" class="btn btn-success"><i class="fa fa-edit"></i></a><?php } ?>
                              <?php if($permisos_efectivos->delete==1) { ?><a onClick="eleminarRegistro('<?php echo base_url().'liquidaciones/liquidaciones_ddis_acreditaciones/delete/'.$result->acreditacion_id ?>')" href="#" class="btn btn-danger"><i class="fa fa-trash-o"></i></a><?php } ?>
                            </div>
                        </td>
                    </tr>
                <?php }?>
