            ...
            <div class="panel-heading">
              <div id="tabs_hdr" class="panel-options">
                <ul class="nav nav-tabs">
                      ...
                      <li><a href="<?=base_url('liquidaciones/liquidaciones_ddis_acreditaciones_detalles/edit/'.$liquidacion_ddi_id)?>">Liq. Acreditaciones</a></li>
                      <li class="active"><a href="<?=base_url('liquidaciones/liquidaciones_ddis_descuentos_detalles/edit/'.$liquidacion_ddi_id)?>">Liq. Descuentos</a></li>
                </ul>
              </div>
            </div>
        <div class="panel-body">
            <div class="tab-content">
              <div id="tab-1" class="tab-pane active">
                <form class="form_liquidaciones_ddis_descuentos_detalles_edit" id="form_liquidaciones_ddis_descuentos_detalles_edit" name="form_liquidaciones_ddis_descuentos_detalles_edit" action="<?php echo base_url('liquidaciones/liquidaciones_ddis_descuentos_detalle/edit/'.$liquidacion_ddi_id)?>" method="post">
                    ...
                    <div class="row">
                        <div class="col-xs-6">
                            <div class="form-group">
                              ...
                              <select id="descuento_id" name="descuento_id" class="form-control filtro-group">
                                <option value="">Seleccione</option>
                                <?php foreach ($descuentos as $descuento): ?>
                                  <option value="<?=$descuento->id ?>"><?php echo $descuento->concepto ?></option>
                                <?php endforeach; ?>
                              </select>
                            </div>
                        </div>
                        ...
                        <tbody id="body-grilla">
                            <?php foreach ($results as $result) { ?>
                                <tr id="resultados" class="nueva">
                                    <td><?=$result->descuento_ddi_detalle_id?></td>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                          <?php if($permisos_efectivos->update==1) { ?><a href="<?php echo base_url().'liquidaciones/liquidaciones_ddis_descuentos_detalle/autorizarDescuentoDdi/'.$result->liquidacion_ddi_id.'/'.$result->descuento_ddi_detalle_id ?>" <?php echo $result->fecha_autorizacion!=NULL?'class="btn btn-success"':'class="btn btn-danger"'?>><i class="fa fa-check-square-o"></i></a><?php } ?>
                                        </div>
                                    </td>
                                </tr>
                            <?php }?>
...
<script type="text/javascript">
   function autorizar_acreditacion_ddi(descuento_ddi_detalle_id){
        $.ajax({
            type: "POST",
            url: '<?php echo base_url()."ajax/liquidaciones/autorizarDescuentoDdi" ?>',
            data: 'descuento_ddi_detalle_id='+descuento_ddi_detalle_id,
            cache: false,
            datatype:'json',
            beforeSend: function() {
            ...
            success: function(respuesta){alert(JSON.stringify(respuesta));
                ...
                if (!obj.status){
                    $('#check_'+descuento_ddi_detalle_id).attr('class', 'btn btn-primary');
                }else{alert("nada");
                    $("<span class='error-tool-tip'>Error al autorizar <span>").insertAfter($('#check_'+descuento_ddi_detalle_id));
...
//    });
    ...
    $('#tipo_concepto_id').change(function(event) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url()."ajax/liquidaciones/getDescuentoGeneral" ?>',
            data: 'descuento_id='+$('#descuento_id').val()+'&tipo_concepto_id='+$('#tipo_concepto_id').val(),
            cache: false,
            datatype:'json',
            beforeSend: function() {
                //$('#errores').html('<h3><i class="fa fa-spinner fa-spin"></i></h3>');
