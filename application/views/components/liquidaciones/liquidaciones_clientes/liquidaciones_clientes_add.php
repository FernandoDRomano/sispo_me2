<div class="col-xs-12">
	...
	<form class="form_liquidacion_add" id="form_liquidacion_add" name="form_liquidacion_add" action="<?php echo base_url('liquidaciones/liquidaciones_clientes/add')?>" method="post" enctype="multipart/form-data">
        ...
        $('.factura_b').click(function(event) {
           ...
           $('.factura_a').click(function(event) {
            ...
            $.each(other_data,function(key,input){
             ...
             $.ajax({
                         url: '<?php echo base_url()."ajax/liquidaciones/getLiquidacionCliente" ?>',
                         dataType: 'json',
                         cache: false,
                         contentType: false,
                         processData: false,
                         ...
                         success: function(data){//alert(JSON.stringify(data));
                             if (data.status){
                                 ...
                                 var cabecera = '<table class="table table-striped table-hover table-condensed bootstrap-datatable table-bordered">'+
                                                    '<thead>'+
                                                        '<tr>'+
                                                            '<th style="width: 15px">Comprobante ing.</th>'+
                                                            '<th>Remito</th>'+
                                                            '<th style="width:100px">Fecha</th>'+
                                                            '<th>Servicio</th>'+
                                                            '<th>Total/Disp.</th>'+
                                                            '<th  style="width:50px">A Liquidar</th>'+
                                                            '<th>Precio</th>'+
                                                            '<th>Importe</th>'+
//                                                            '<th>Ajustes</th>'+
                                                        '</tr>'+
                                ...
                                $('#cantidad_elementos').val(data['grilla'].length);
                                $.each(data['grilla'], function(index, element) {
                                      var precio = parseFloat(element['precio']).toFixed(3);
                                      var importe = (parseFloat(element['precio']) * parseFloat(element['cantidad'] - element['cant_liquidada'])).toFixed(3);
                                      var cant_disponible =  element['cantidad'];
                                      var cant_liquidada = 0;
                                      if (element['cant_liquidada'] !== null){
                                          cant_disponible = element['cantidad'] - element['cant_liquidada'];
                                          cant_liquidada = element['cant_liquidada'];
                                      }
                                      body    += '<tr id="resultados" class="nueva"><input type="hidden" name="fila_'+contador+'" value="'+element['comprobante_ingreso_id']+'|'+element['remito']+'|'+element['numero']+'|'+element['remito']+'|'+element['fecha']+'|'+element['servicio_id']+'|'+element['servicio_especial']+'|'+element['cantidad']+'|'+cant_disponible+'|'+precio+'"/>'+
                                                    '<td class="width:10px">'+element['numero']+'</td>'+
                                                    '<td class="">'+element['remito']+'</td>'+
                                                    '<td style="width:100px">'+element['fecha']+'</td>'+
                                                    '<td class="">'+element['servicio_especial']+'</td>'+
                                                    '<td style="width:80px;text-align:right">'+element['cantidad']+'/'+cant_disponible+'</td>'+
                                                    '<td><input style="width:80px;text-align:right" type="text" name="cantidad_liquidar_'+contador+'" id="cantidad_liquidar_'+contador+'" value="'+cant_disponible+'"\></td>'+
                                                    '<td style="width:80px;text-align:right"><span style="float:left">$</span><span style="float:right;text-align:left">'+precio+'</span></td>'+
                                                    '<td style="width:80px;text-align:right"><span style="float:left">$</span><span style="float:right;text-align:right">'+importe+'</span></td>'+
//                                                    '<td><a href="javascript:;" onclick="agregarAjuste(this,'+element["numero"]+','+element["servicio_id"]+')" class="btn btn-successfull"><i class="fa fa-plus"></i></a></td>'+
                                                '</tr>';
                                    // }
                                 ...
                                 var total = '<tr>'+
                                                ...
                                                '<td class=""></td>'+
                                                '<td class=""></td>'+
                                                '<td style="text-align:right;font-weight: bold"><strong>Sub-Total:</strong></td>'+
                                                '<td style="text-align:right;font-weight: bold"><span style="float:left">$</span><span style="float:right;text-align:left"><strong>'+suma_total.toFixed(3)+'</strong></span></td>'+
                                            '</tr>'+
                                            '<tr>'+
                                                '<td class=""></td>'+
                                                '<td class=""></td>'+
                                                ...
                                                '<td style="text-align:right;font-weight: bold"><span style="float:left">$</span><span style="float:right;text-align:left"><strong>'+iva.toFixed(3)+'</strong></span></td>'+
                                            '</tr>'+
                                            '<tr>'+
                                                '<td class=""></td>'+
                                                '<td class=""></td>'+
                                                ...
                                                '<td style="text-align:right;font-weight: bold"><span style="float:left">$</span><span style="float:right;text-align:left"><strong>'+total_iva.toFixed(3)+'</strong></span></td>'+
                                            '</tr>';
                                 $('#grilla').append(cabecera+body+total+cierre); 
                                 barcodear();
                                 //Tabla Resumen
