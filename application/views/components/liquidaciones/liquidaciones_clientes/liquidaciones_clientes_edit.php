<div class="col-xs-12">
	...
	<form class="form_liquidacion_add" id="form_liquidacion_add" name="form_liquidacion_clientes_add" action="<?php echo base_url('liquidaciones/liquidaciones_clientes/edit/'.$result->id)?>" method="post" enctype="multipart/form-data">
                        ...
                        <?php   $suma_total = 0;
                                ...
                                foreach ($detalles as $detalle) {?>
                                    <?php   $signo = (double)$detalle->cant_a_liquidar < 0?-1:1;
                                            $importe = (int)$detalle->cant_a_liquidar * (float)$detalle->precio *(int)$signo;
                                    ...
                                    <tr id="resultados" class="nueva"><input type="hidden" name="fila_<?=$contador?>" value="<?=$contador?>"/>
                                        <td style="text-align:left;<?php echo $estilo?>"><?=$detalle->comprobante_ingreso?></td><input type="hidden" id="numero_<?=$contador?>" name="numero_<?=$contador?>" value="<?=$detalle->comprobante_ingreso?>"/>
                                        <td style="text-align:left;<?php echo $estilo?>"><?=$detalle->remito_cliente?></td><input type="hidden" id="numero_<?=$contador?>" name="numero_<?=$contador?>" value="<?=$detalle->remito_cliente?>"/>
                                        <td style="text-align:left;<?php echo $estilo?>"><?=$detalle->fecha_liq?></td></td><input type="hidden" id="fecha_<?=$contador?>" name="fecha_<?=$contador?>" value="<?=$detalle->fecha?>"/>
                                        <td style="text-align:left;<?php echo $estilo?>"><?=$detalle->servicio?></td></td><input type="hidden" id="servicio_<?=$contador?>" name="servicio_<?=$contador?>" value="<?=$detalle->servicio?>"/>
                                        <td style="text-align:right;<?php echo $estilo?>"><?=$detalle->cant_a_liquidar?></td></td><input type="hidden" id="cantidad_<?=$contador?>" name="cantidad_<?=$contador?>" value="<?=$detalle->cantidad?>"/>
                                        <td style="text-align:right;<?php echo $estilo?>"><span style="float:left">$</span><span style="float:right;text-align:left"><?=$detalle->precio?></span></td></td><input type="hidden" id="precio_<?=$contador?>" name="precio_<?=$contador?>" value="<?=$detalle->precio?>"/>
                                        <td style="text-align:right;<?php echo $estilo?>"><span style="float:left">$</span><span style="float:right;text-align:left"><?=number_format($importe,3);?></span></td></td><input type="hidden" id="importe_<?=$contador?>" name="importe_<?=$contador?>" value="<?=number_format((int)$detalle->cantidad * (float)$detalle->precio,3);?>"/>
                                        <!--<td><a href="javascript:;" onclick="agregarAjuste(<?=$detalle->comprobante_ingreso?>,<?=$detalle->servicio_id?>)" class="btn btn-successfull"><i class="fa fa-plus"></i></a>-->
                                        <?php if((int)$detalle->liquidacion_ajuste_id === 0){?>
                                            <td><a data-toggle="modal" href="<?php echo base_url().'liquidaciones/liquidaciones_clientes/addAjustes/'.$detalle->comprobante_ingreso.'/'.$detalle->servicio_id.'/'.$detalle->liquidacion_cliente_id.'/'.$detalle->liquidacion_comprobante_id.'/'.$detalle->precio?>" data-target="#myModal" class="btn btn-info" <?php echo $result->estado_id == 2|| ($tipo_factura != "A")?'disabled':''?>><i class="fa fa-edit"></i></a></td>
                                        <?php }else{ ?>
                                            <td><a href="<?php echo base_url().'liquidaciones/liquidaciones_clientes/deleteAjuste/'.$result->id.'/'.$detalle->liquidacion_ajuste_id?>" class="btn btn-danger" <?php echo $result->estado_id == 2|| ($tipo_factura != "A")?'disabled':''?>><i class="fa fa-trash-o"></i></a></td>
                                        <?php }?>     
                                     </tr>
                                     <?php $contador++;
                                            $suma_total = (float)($suma_total)+(float)((int)$detalle->cantidad * (float)number_format($detalle->precio,3));
                                };
                                $iva = $importe_subtotal*0.21;
                                $importe_total = ($importe_subtotal + $iva);
                                ?>
                                <?php if($tipo_factura == "A"){?>
                                     <tr>
                                         <td colspan="6" style="text-align:right;font-weight: bold">SUBTOTAL: </td>
                                         <td  style="text-align:right;font-weight: bold"><span style="float:left">$</span><span style="float:right;text-align:left"><?=number_format($importe_subtotal,3);?></span></td> 
                                         <td></td>
                                     </tr>
                                     <tr>
                                        <td colspan="6" style="text-align:right;font-weight: bold">IVA: </td>
                                        <td style="text-align:right;font-weight: bold"><span style="float:left">$</span><span style="float:right;text-align:left"><?=number_format($iva,3);?></span></td>  
                                        <td></td>
                                     </tr>
                                     <tr>
                                         <td colspan="6" style="text-align:right;font-weight: bold">TOTAL: </td>
                                        <td style="text-align:right;font-weight: bold"><span style="float:left">$</span><span style="float:right;text-align:left"><?=number_format($importe_total,3);?></span></td> 
                                        <td></td>
                                     </tr>
                                <?php }?>
                ...
                <div id="grilla_resumen">
                    <table class="table table-striped table-hover table-condensed bootstrap-datatable table-bordered" style="width:50%">
                        ...
                        <tbody id="body-grilla-resumen">
                            <?php foreach ($resumen as $valor) {?>
                            <tr id="" class="nueva">
                                <td class="barcodear"><?=$valor->servicio_id?></td>
                                <td class=""><?=$valor->servicio?></td>
                                <td class=""><?=$valor->cant_a_liquidar?></td>
                            </tr>
                            <?php }?>
                        </tbody>
                    </table>
             ...
             $.ajax({
                         ...
                         success: function(data){
                             if (data.status){
                                ...
                                $('#cantidad_elementos').val(data['grilla'].length);
                                $.each(data['grilla'], function(index, element) {
                                      var precio = parseFloat(number_format(element['precio'],3));
