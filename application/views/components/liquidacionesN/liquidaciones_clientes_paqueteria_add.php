...
<div class="col-xs-12">
    <div class="ibox-content">
        <form class="form_liquidacion_cliente_paqueteria_add" id="form_liquidacion_cliente_paqueteria_add" name="form_liquidacion_cliente_paqueteria_add" action="<?php echo base_url('liquidacionesN/liquidaciones_clientes_paqueteria/add') ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-xs-3">
                    <div class="form-group">
                        ...
                        <input id="fecha_ci_desde" name="fecha_ci_desde" type="text" class="form-control datepicker" placeholder="Seleccione una fecha" value="<?php if ($fecha_ci_hasta) {
                                                                                                                                                                    $f = strtotime($fecha_ci_hasta);
                                                                                                                                                                    echo date('d-m-Y', $f);
                                                                                                                                                                } ?>" autocomplete="off" />
                    </div>
                </div>
                <div class="col-xs-3">
                    <div class="form-group">
                        ...
                        <input id="fecha_ci_hasta" name="fecha_ci_hasta" type="text" class="form-control datepicker" placeholder="Seleccione una fecha" value="<?php if ($fecha_ci_hasta) {
                                                                                                                                                                    $f = strtotime($fecha_ci_hasta);
                                                                                                                                                                    echo date('d-m-Y', $f);
                                                                                                                                                                } ?>" autocomplete="off" />
                    </div>
                ...
                success: function(data) { //alert(JSON.stringify(data));
                    ...
                    if (data.status) {
                        ...
                        $.each(data['grilla'], function(index, element) {
                            ...
                            //let precio_unitario = data['precios_unitario'][i] ?? 0;
                            //let precio_valor_declarado = data['valor_declarado'][i] ?? 0;
                            //let precio_cobranza = data['cobranza'][i] ?? 0;
                            ...
                            //var cantidad_bultos = element['bultos'] == null ? 1 : element['bultos'];
                            /* FIN DEL AGREGADO */
                            
                            /*
                            body += '<tr id="resultados" class="nueva"><input type="hidden" name="fila_' + contador + '" value="' + element['comprobante_ingreso_id'] + '|' + element['remito'] + '|' + element['numero'] + '|' + element['remito'] + '|' + element['fecha'] + '|' + element['servicio_id'] + '|' + element['servicio_especial'] + '|' + cantidad + '|' + cant_disponible + '|' + precio + '"/>' +
                                ...
                                '<td class="">' + dimensiones + '</td>' + //aqui dimensiones
                                '<td class="">' + (element['peso'] ?? '-') + '</td>' +
                                '<td class="">' + (element['peso_volumetrico'] ?? '-') + '</td>' +
                                '<td class="">' + (element['bultos'] ?? '-') + '</td>' +
                                '<td class="">' + (element['palets'] ?? '-') + '</td>' +
                                '<td class="">' + (element['metro_cubico'] ?? '-') + '</td>' +
                                '<td class="">' + element['barcode_externo'] + '</td>' +
                                '<td style="width:80px;text-align:right">' + (parseFloat(data['precios_unitario'][i])).toFixed(2) + '</td>' +
                                '<td style="width:80px;text-align:right">' + (parseFloat(data['valor_declarado'][i])).toFixed(2) + '</td>' +
                                '<td style="width:80px;text-align:right">' + (parseFloat(data['valor_calculado'][i])).toFixed(2) + '</td>' +
                                '</tr>';
                            */
