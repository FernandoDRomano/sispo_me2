<div class="col-xs-12">
    <div class="ibox-content">
        <?php if($this->session->flashdata('registro')): ?>
          ...
          <?php endif; ?>
          <form action="<?=base_url('liquidacionesN/liquidaciones_clientes_paqueteria/filtro')?>" method="post" id="form-filtro">
            <div class="row">
              <div class="col-xs-3">
            ...
            <div  class="row">
                ...
                <div class="col-xs-2" style="padding-top:23px">
                    <div class="btn-group">
                      ...
                      <a href="<?=base_url('liquidacionesN/liquidaciones_clientes_paqueteria')?>" class="btn btn-primary" title="Reinciar"><i class="fa fa-refresh"></i></a>
                    </div>
                </div>
            </div>
          </form>
        <div class="text-right">            
            <?php if($permisos_efectivos->insert==1) { ?> <a href="<?php echo base_url().'liquidacionesN/liquidaciones_clientes_paqueteria/add/' ?>" class="btn btn-success"><i class="fa fa-plus"></i> Nueva Liquidaci贸n</a><?php } ?><hr>
        </div>
              ...
              <?php foreach ($liquidaciones as $result) { 
                  ...
                  <tr>
                    ...
                    <td><?php if($result->estado_id == 0) echo "PENDIENTE";
                              if($result->estado_id == 1) echo "PROCESO";
                              if($result->estado_id == 2) echo "CERRADA";?></td>
                    <td>
                      <div class="btn-group">
<!--                        <a data-toggle="modal" href="<?php echo base_url().'liquidacionesN/liquidaciones_clientes_paqueteria/view/'.$result->id ?>" data-target="#myModal" class="btn btn-info"><i class="fa fa-search"></i></a>-->
              <?php if($permisos_efectivos->update==1 && $result->estado_id) { ?><a href="<?php echo base_url().'liquidacionesN/liquidaciones_clientes_paqueteria/edit/'.$result->id ?>" class="btn btn-success"><i class="fa fa-edit"></i></a><?php } ?>
                        <?php if($permisos_efectivos->delete==1) { ?><a onClick="anularLiquidacion('<?php echo base_url().'liquidacionesN/liquidaciones_clientes_paqueteria/anular/'.$result->id ?>')" href="#" class="btn btn-danger"><i class="fa fa-trash-o"></i></a><?php } ?>
                      </div>
                    </td>
                  </tr>
              <?php } ?>
...
<script type="text/javascript">
    function anularLiquidacion(link){
      bootbox.confirm("Desea borrar la liquidación?", function(result) {
        if (result === true) {
            $.ajax({
                type: "GET",
                url: link,
