...
<div class="col-xs-12">
    <div class="ibox-content">
        <form class="form_liquidacion_paqueteria_edit" id="form_liquidacion_paqueteria_edit" name="form_liquidacion_paqueteria_edit" action="<?php echo base_url('liquidacionesN/liquidaciones_clientes_paqueteria/edit/' . $result->id) ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-xs-3">
                    <div class="form-group">
                        ...
                        <input id="periodo_desde" name="periodo_desde" type="text" class="form-control datepicker" placeholder="Seleccione una fecha" value="<?php if ($result->periodo_desde) {
                                                                                                                                                                    $f = strtotime($result->periodo_desde);
                                                                                                                                                                    echo date('d-m-Y', $f);
                ...
                <div class="col-xs-3">
                    <div class="form-group">
                        ...
                        <input id="periodo_hasta" name="periodo_hasta" type="text" class="form-control datepicker" placeholder="Seleccione una fecha" value="<?php if ($result->periodo_hasta) {
                                                                                                                                                                    $f = strtotime($result->periodo_hasta);
                                                                                                                                                                    echo date('d-m-Y', $f);
                                                                                                                                                                } ?>" required autocomplete="off" readonly />
                    </div>
                </div>
                ...
                <div class="controls">
                    <div class="row">
                        <div class="col-xs-8" style="text-align: left">
                            <!--                               <a class="btn btn-primary factura_b" href="<?php echo base_url() . 'liquidacionesN/liquidaciones_clientes/edit/' . $result->id ?>" ><i class="fa fa-arrow-down"></i> Factura A</a>
                               <a class="btn btn-primary factura_b" href="<?php echo base_url() . 'liquidacionesN/liquidaciones_clientes/edit_factura_b/' . $result->id ?>" ><i class="fa fa-arrow-down"></i> Factura B</a>
                                  <a class="btn btn-primary liquidacion_x" href="<?php echo base_url() . 'liquidacionesN/liquidaciones_clientes/edit_factura_x/' . $result->id ?>" ><i class="fa fa-arrow-down"></i> Liquidación X</a>
                        ...
                        <div class="col-xs-4" style="text-align: right">
                            ...
                            <?php } ?>
                            <!--<a onClick="confirmarLiquidacion('<?php echo base_url() . 'liquidacionesN/liquidaciones_clientes/confirmar/' . $result->id ?>')" href="#" class="btn btn-success" <?php echo $result->estado_id == 2 ? 'disabled' : '' ?>><i class="fa fa-check-square-o"></i>Confirmar Liquidación</a>-->
                        </div>
                    </div>
                </div>
                <div>&nbsp;</div>
                        ...
                        <tbody id="body-grilla">
                            ...
                            foreach ($detalles as $detalle) { ?>
                                ...
                                <tr id="resultados" class="nueva"><input type="hidden" name="fila_<?= $contador ?>" value="<?= $contador ?>" />
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= $detalle->numero ?></td>
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= $detalle->remito_cliente ?></td>
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= strlen($detalle->codigo) == '0' ? '-' : $detalle->codigo ?></td>
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= $detalle->fecha ?></td>
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= $detalle->descripcion_paquete ?></td>
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= $detalle->dimensiones ? $detalle->dimensiones : '-' ?></td>
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= $detalle->metro_cubico ? '-' : ($detalle->peso ? $detalle->peso : '-')  ?></td>
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= $detalle->peso_volumetrico ? $detalle->peso_volumetrico :  '-' ?></td>
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= $detalle->bultos ? $detalle->bultos :  '-' ?></td>
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= $detalle->palets ? $detalle->palets :  '-' ?></td>
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= $detalle->metro_cubico ? $detalle->metro_cubico :  '-' ?></td>
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= $detalle->nombre_gestion_flota ? $detalle->nombre_gestion_flota . ' | ' . $detalle->cantidad_hora . 'hs' : '' ?></td>
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= $detalle->precio_unitario ? $detalle->precio_unitario :  '-' ?></td>
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= $detalle->precio_valor_declarado ? $detalle->precio_valor_declarado :  '-' ?></td>
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= $detalle->precio_cobranza ? $detalle->precio_cobranza :  '-' ?></td>
                                    <td style="text-align:center;<?php echo $estilo ?>"><?= $detalle->precio; ?></td>
                                </tr>
                            <?php $contador++;
                            ...
                            <tr>
                                <td colspan="15" style="text-align:right;font-weight: bold">SUBTOTAL: </td>
                                <td style="text-align:right;font-weight: bold"><?= number_format($suma_total, 2); ?></td>
                            </tr>
                            <tr>
                                <td colspan="15" style="text-align:right;font-weight: bold">IVA: </td>
                            ...
                            <tr>
                                ...
                                <td style="text-align:right;font-weight: bold"><?= number_format($importe_total, 2); ?></td>
                            </tr>

                            <?php if ($tipo_factura == "B") { ?>
                                <tr>
                                    <td colspan="6" style="text-align:right;font-weight: bold">TOTAL: </td>
                                    <td style="text-align:right;font-weight: bold"><?= number_format($importe_subtotal, 2); ?></td>
                                    <td></td>
                                </tr>
                            <?php } ?>
...
<form method='post' action='<?php echo base_url('liquidacionesN/liquidaciones_clientes_paqueteria/exportarAPdf') ?>' id='exportAPdf'>
