$(document).ready(function() {
    ...
    $('#servicio_id').change(function(event) {
        ...
        $.getJSON('<?=base_url()?>' + 'ajax/piezas/getLinksImpresionAcusesEtiquetas/' + $('#comprobante_id').val()+'/'+$('#servicio_id').val(), function(data) {
            ...
            var links_paqueteria = 'Paqueteria: ';
            if(!data.status){
                var i = 0;
                ...
                links_paqueteria = links_paqueteria + '<a href="http://mailenvios.sppflash.com.ar/sispopdf?servicio=' + $('#servicio_id').val()  + '&comprobante=' + $('#comprobante_id').val()  + '" target="_blank" >Libro Unico</a> ';
                    
            }

            $("#div_acuses").append(links_acuses);
            $("#div_etiquetas").append(links_etiquetas);
            $("#div_paqueteriamail").append(links_paqueteria);
