...
//    $('#servicio').change(function(event) {alert($('#servicio').val());
...
//    });
    ...
    $('.agregar-grilla').click(function(event) {
        ...
        if($('#servicio').val() > 0 )
        {
            ...
            var html = '<tr id="grilla-servicio-'+total+'"><td>'+servicio_txt+inputs+'</td><td>'+cantidad+'</td><td>'+remito+'</td><td><a href="javascript:;" onclick="eliminarFila(\'grilla-servicio-'+total+'\')" class="btn btn-danger"><i class="fa fa-trash-o"></i></a></td></tr>';
...
function liberar_comprobante(id){
    bootbox.confirm("Esta acción borrará todos los servicios y sus piezas asociadas quedando libre el comprobante para su utilización.<br/>¿Desea Continuar?", function(result) {
        if (result === true) {
            $('#liberar_comprobante_id').val($('#comprobante_ingreso_id').val());
            $('#liberar_comprobante').submit();
//            $.ajax({
//                type: "GET",
//                url: link,
//                data: {},
