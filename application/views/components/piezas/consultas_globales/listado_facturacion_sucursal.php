<div class="col-xs-12">
    <div class="ibox-content">
        <form class="form_listado_facturacion_sucursal" id="form_listado_facturacion_sucursal" name="form_listado_facturacion_sucursal" action="<?php echo base_url('consultas/add')?>" method="post" enctype="multipart/form-data">
...
<form method='post' action='<?php echo base_url('consultas/Listado_facturacion_sucursal/exportarAExcel')?>' id='exportarAExcel'>
...
$(document).ready(function() {
    ...
    $('.aceptar').click(function(event) {
        $('#grilla').html('');
        var other_data = $('#form_listado_facturacion_sucursal').serializeArray();
        var form_data = new FormData();
        $.each(other_data,function(key,input){
            form_data.append(input.name,input.value);
        });
         $.ajax({
                url: '<?php echo base_url()."ajax/consultas/getListadoFacturacionSucursal" ?>',
                dataType: 'json',
                cache: false,
                contentType: false,
                processData: false,
                ...
                success: function(data){//alert(JSON.stringify(data));
                    if (data.status){ alert("No se encontro resultados");
                        ...
                        var html = '<table width="100%" border="1" style="table-layout: fixed;" class="table-striped table-hover table-condensed bootstrap-datatable table-bordered">'+
                                          ...
                                          '</thead>'+
                                          '<tbody>';
                                            $.each(data['facturacion_sucursal'], function(index, element) {//alert(JSON.stringify(element['cantidad']));
                                              html_filas += '<tr>'+
                                                                  '<td>'+element['sucursal']+'</td>'+
                                                                  '<td style="text-align:right">'+element['cantidad']+'</td>'+
                                                                  '<td style="text-align:right">'+element['facturacion']+' ($)</td>'+
                                                              '</tr>';
                                                i++;
                                            });
