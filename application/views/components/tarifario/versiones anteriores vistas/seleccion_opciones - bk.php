...
<!--WareHouse-->
					<div class="ibox-content" id="posicion_precio" name="c_posicion_precio" style="display: none;">
						<div class="row campoDinamico_3campos">
							<label>Ingrese los valores por favor.</label><br><br>
							<div class="col-xs-3">
								<div class="form-group">
									<label>Posicion</label>
									<input id="posicion1" placeholder="Ej.: 10" type="number" name="c_posicion1" onkeyup="revisionCamposLlenos()" class="form-control filtro-group">
								</div>
							</div>
							<div class="col-xs-3">
								<div class="form-group">
									<label>Precio</label>
									<input placeholder="$" type="number" id="precioW1" name="c_precioW1" onkeyup="revisionCamposLlenos()" class="form-control filtro-group" step="0.01">
								</div>
							</div>
							<!--Boton + (para agregar campos) -->
							<div class="col-xs-1">
...
<script type="text/javascript">
	/*
	var resultado_grabado = <?php echo (json_encode($resultado_grabado)); ?>;
 
 	console.log('/RESULTADO GRABADO/' +resultado_grabado); 

 	var observacion_grabado = <?php echo (json_encode($observacion_grabado)); ?>;
 	console.log('/RESULTADO OBSERVACION/' +observacion_grabado);
	*/
		...
		if (seleccion == 1) {
			...
			switch (valueComboOpciones) {
				...
				case 8:
					...
					posicion_precio.style.display = "block";
					break;
				default:
			...
			if (countX < maxField) {
				...
				switch (valueComboOpciones) {
					...
					case 6: //Palets
						...
						break;
					case 8: //Warehouse
						var wrapper = $('#posicion_precio');
						var fieldHTML = '<div class="row campoDinamico_3campos campo_agregado"><div class="col-xs-3"><div class="form-group"><label>Posicion</label><input id="posicion' + countX + '" placeholder="Ej.: 10" type="number" name="c_posicion' + countX + '" onkeyup="revisionCamposLlenos()" class="form-control filtro-group"></div></div><div class="col-xs-3"><div class="form-group"><label>Precio</label><input placeholder="$" type="number" id="precioW' + countX + '" name="c_precioW' + countX + '" onkeyup="revisionCamposLlenos()" class="form-control filtro-group" step="0.01"></div></div><div class="col-xs-1"><div class="form-group"><br><a href="javascript:void(0);" class="remover_campos btn btn-success btn-submit" title="Remove field" name="c_signoMenos">-</a></div></div></div>'
	...
	function modificar_nombres_camposDinamicos(boolean) {
		...
		switch (valueComboOpciones) {
			...
			case 8:
				//Declaracion de Variables (string)
				var nombreEdit1Id_posicion = 'posicion';
				var nombreEdit1Name_posicion = 'c_posicion';

				var nombreEdit1Id_precio = 'precioP';
				var nombreEdit1Name_precio = 'c_precioP';

				var nombreEdit2Id_posicion = '';
				var nombreEdit2Name_posicion = '';

				var nombreEdit2Id_precio = '';
				var nombreEdit2Name_precio = '';


				$('.campoDinamico_3campos').children('div').children('div').find('input').each(function() {
					countRecorrido = countRecorrido + 1;

					switch (countRecorrido) {
						case 1:
							nombreEdit2Id_posicion = nombreEdit1Id_posicion + countNombresCampos;
							$(this).attr('id', nombreEdit2Id_posicion);

							nombreEdit2Name_posicion = nombreEdit1Name_posicion + countNombresCampos;
							$(this).attr('name', nombreEdit2Name_posicion);
							break;
						case 2:
							nombreEdit2Id_precio = nombreEdit1Id_precio + countNombresCampos;
							$(this).attr('id', nombreEdit2Id_precio);
	...
	function revisionCamposLlenos() {
		...
		switch (valueComboOpciones) {
			...
			case 8
				...
				//Declaracion de Variables
				var valorCampoPosicion = 0;
				var valorCampoPrecio = 0;

				var banderaPosicion = false;
				var banderaPrecio = false;
				...
				$('.campoDinamico_3campos').children('div').children('div').find('input').each(function() {
					...
					switch (countRecorrido) {
						case 1:
							valorCampoPosicion = $(this).val();

							if (valorCampoPosicion != 0 && valorCampoPosicion != '') {
								banderaPosicion = true;
							} else {
								banderaPosicion = false;
							}
							matriz_valoresInterna.push(valorCampoPosicion);
						...
						case 2:
							...
							if (banderaCantidad && banderaPrecio) {
								...
								array_ingresos.push(valorCampoPosicion, valorCampoPrecio);
								matriz_ingresos_Externa.push(array_ingresos);
								array_ingresos = [];
							} else {
								valoresIngresados = false;
