...
class Clientes_precios_especiales extends CI_Controller
{
	...
	function guardarPreciosEspecialesPorcentajes()
	{
		...
		$actualizar_por = $this->input->post("actualizar_por");
		...
		if ($servicios_id != NULL && ($porcentaje != "" ||  $importe != "")) {
			...
			//id insertado en flash_actualizacion_precios
			$actualizacion = $this->codegen_model->add('flash_actualizacion_precios', $data);
			$precios_especiales = $this->codegen_model->get('flash_clientes_precios_especiales', '*', 'cliente_id = ' . $cliente_id . ' AND servicio_id IN (' . implode(",", $servicios_id) . ')');
			//echo($this->db->last_query());die;  

			foreach ($precios_especiales as $f) {
				if ($actualizar_por == 'por') {
					$precio_nuevo = floatval($f->precio) + (floatval($f->precio) * floatval($porcentaje) / 100);
				} else if ($actualizar_por == 'imp') {
					$precio_nuevo = floatval($importe);
				}
