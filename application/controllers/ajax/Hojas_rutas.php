    ...
    public function getHDRPendientesXClientes()
    {
        ...
        $query = $this->db
            ->select('suc.nombre sucursal, ci.cantidad cantidad,DATE_FORMAT(ci.create,"%d-%m-%Y") fecha_ingreso,
                                ...
                                p.domicilio, L.nombre as localidad_destino, p.barcode_externo codigo_externo
                                ')
...
//            echo $this->db->last_query();die;
        ...
        public function add_piezas_simples(){
            ...
            $sql = " INSERT  INTO flash_subpiezas (hoja_ruta_id, pieza_id, liquidacion_id, cantidad, pieza_estado_id, create_user_id, flash_subpiezas.create) 
                    SELECT 
    ...
    public function getPiezaNormal()
    {
        ...
        $query = $this->db
            ->select('p.barcode_externo barcode_externo,p.id pieza_id, s.nombre servicio, p.domicilio domicilio, p.codigo_postal_destino codigo_postal,L.nombre as localidad_destino, p.destinatario, c.nombre cliente')
            ->join('flash_comprobantes_ingresos ci', 'ci.id = p.comprobante_ingreso_id')
            ->join('flash_clientes c', 'c.id = ci.cliente_id')
    ...
    public function traerPiezasSimplesPorComprobanteEstadosYServicio()
    {
        ...
        $query = $this->db
            ->select(' p.barcode, p.id, p.destinatario, p.domicilio, L.nombre as localidad_destino, p.codigo_postal_destino, s.nombre as servicio, p.barcode_externo')
            ->join('flash_comprobantes_ingresos_servicios cis', 'p.servicio_id = cis.id')
