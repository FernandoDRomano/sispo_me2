...
class Consulta_global extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($procesar != '1') {
            ...
            $total_records = $this->flash_md->getConsultasGlobalesTotal($fecha_ingreso, $pieza_id, $barra_externa, $comprobante, $cliente, $servicio, $cartero,
                $hoja_ruta_id, $despacho_id, $sucursal, $estado, $destinatario, $domicilio, $codigo_postal, $localidad,
                $fecha_cambio_estado, $visitas, $rendicion_id, $recibio, $documento, $vinculo, $datos_varios_1, $datos_varios_2, $datos_varios_3, $datos_varios_4, $fecha_pieza_desde, $fecha_pieza_hasta);
//var_dump(total_records);die;
            if ($total_records > 0) {
                // get current page records
                $vista_interna["results"] =  $this->flash_md->getConsultasGlobalesPaginado($fecha_ingreso, $pieza_id, $barra_externa, $comprobante, $cliente, $servicio, $cartero,
                $hoja_ruta_id, $despacho_id, $sucursal, $estado, $destinatario, $domicilio, $codigo_postal, $localidad,
                $fecha_cambio_estado, $visitas, $rendicion_id, $recibio, $documento, $vinculo, $datos_varios_1, $datos_varios_2, $datos_varios_3, $datos_varios_4, $fecha_pieza_desde, $fecha_pieza_hasta, $limit_per_page, $start_index);
                $i = 0;
        ...
        $vista_externa = array(
            ...
            'contenido_main' => $this->load->view('components/piezas/consultas_globales/consultas_globales_list', $vista_interna, true),
        );
    ...
    public function exportarAExcel()
    {
        ...
        $this->excel->getActiveSheet()->setCellValue("R1", 'Rendici��n');
        $this->excel->getActiveSheet()->setCellValue("S1", 'Recibi��');
        $this->excel->getActiveSheet()->setCellValue("T1", 'Documento');
        $this->excel->getActiveSheet()->setCellValue("U1", 'V��nculo');
        ...
        $this->excel->getActiveSheet()->setCellValue("Y1", 'Datos Varios 3');
        $this->excel->getActiveSheet()->setCellValue("Z1", 'mail');
        $this->excel->getActiveSheet()->setCellValue("AA1", 'celular');
        // set cells values
        ...
        foreach ($consultas_globales as $value) {
            ...
            $this->excel->getActiveSheet()->setCellValue("Y$i", $value->datos_varios_3);
            $this->excel->getActiveSheet()->setCellValue("Z$i", $value->mail_dest);
            $this->excel->getActiveSheet()->setCellValue("AA$i", $value->celular_dest);
            $i++;
        }
