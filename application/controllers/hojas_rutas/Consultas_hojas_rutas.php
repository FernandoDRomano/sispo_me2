...
//            echo $dias[date('w')]." ".date('d')." de ".$meses[date('n')-1]. " del ".date('Y')."<br/>" ;
    ...
    function exportarHDRCartero($hoja_ruta,$auditoria){
        ...
        $pdf->Write(5, 'CodeIgniter TCPDF Integration'); 
            ...
            $total = 0;
            //var_dump($hdr_piezas_normales);die;
            ...
            if($auditoria == 'true'){
                ...
                $titulo = ' <table border="0" cellspacing="6" cellpadding="4" style="margin-bottom:20px">
                                <tr>
                                    ...
                                    <td style="font-size:11px">Recorrido <br/>'.isset($hdr_piezas_normales->zona).'</td>
                ...
                //var_dump($hdr_piezas_normales_auditoria);die;
                foreach($hdr_piezas_normales_auditoria as $piezas){
                   $tabla_auditoria_contenido  .= '<tr>
                                                        ...
                                                        <td>'.$piezas->servicio.'<br/>'.$piezas->barcode_externo.' / '.$piezas->pieza_id.'<br/>'.isset($piezas->cliente).'</td>
                                                    </tr>
                                                    <tr>
                                                        <td></td>
                                                        <td>'.$piezas->remito.', '.$piezas->datos_varios_2.', '.$piezas->datos_varios_3.'</td>
                                                    </tr>
...
//            header('Location: '.$url);
    ...
    function hdr_cartero_distribuidor(){
            ...
            else if( $this->ion_auth->user()->row()->id == 224 || $this->ion_auth->user()->row()->id == 195){ //user amendoza y auditoriatuc puede ver todas las sucursales
