...
//            var_dump($liquidacion);die;
            ...
            $this->excel->getActiveSheet()->getStyle('A1:A1000')->applyFromArray($styleArray);
...
//                    $this->excel->getActiveSheet()->setCellValue("C$i",$value->hojas);
    ...
    //            echo $query_carteros;die;
               ...
               foreach ($carteros as $cartero) {
                   ...
                   // var_dump($liquidacion_resumen_clientes);die;
                    for($i = 0; $i < count($liquidacion_resumen_clientes_array); $i++){

                        //Primero veo si tiene valor especial por cliente y cartero
                        $query_valor_especial_entrega = "SELECT ve.cliente_id, c.nombre cliente, ve.pago valor_especial_entrega
                                           ...
                                           //    var_dump($query_valor_especial_entrega);die;
...
//                        if ($cartero->cartero_id  == 26 && $liquidacion_resumen_clientes[$i]->cliente_id ==3){echo $query_piezas;die;}
                        ...
                        if ($valor_especial_entrega != NULL){//if ( $cartero->cartero_id) {echo "1";die;}
                        ...
                        if ($valor_entrega->valor_entrega == NULL || $valor_entrega->valor_entrega == '' ){//if ( $cartero->cartero_id) {echo "3";die;}
                            $valor = $predeterminado->valor;//if ( $cartero->cartero_id==456) {echo "1 ".$valor;die;}
                        } else{
                            $valor = $valor_entrega->valor_entrega;//if ( $cartero->cartero_id==456) {echo "3 ". $valor;die;}
                        }
                                $col_aux = $columna++;
                   ...
                   // if($cartero->cartero_id == 456) {var_dump($piezas_valor_array);die;}
                    ...
                    ////////Productividad MAYOR que CERO
                        if ($productividad->productividad > 0){
                            foreach ($piezas_valor_array as $key => $value) {//if ( $cartero->cartero_id == 456) {var_dump($key);var_dump($value);die."<br>";}
                                if ( $suma < $productividad->productividad){
                                    ...
                                    if($suma >= $productividad->productividad && $primera_pasada){
                                        ...
                                        $total_a_pagar += $value['valor'] * $value['cantidad'];
                                        //if ( $cartero->cartero_id == 456) {echo "segundas pasadas total a pagar ". $total_a_pagar."<br>";}
                                      // if ( $cartero->cartero_id == 456) {echo count($piezas_valor_array)."<br>";echo $suma."..8888888888..".$productividad->productividad."<br>";}die;
                                    }
                                }
                                //if ( $cartero->cartero_id == 456) {count($piezas_valor_array);echo $suma."....".$productividad->productividad."<br>";}
                                //if ( $cartero->cartero_id == 456) {echo $valor ."  ---   ".$cantidad  ; die; }
                            }
                        }
