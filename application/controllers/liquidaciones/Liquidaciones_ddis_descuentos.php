...
class Liquidaciones_ddis_descuentos extends CI_Controller {
	...
	function index(){
                $query = "SELECT ld.id descuento_id, sc.id cartero_id, sc.apellido_nombre cartero, concepto, 
                                    ld.observaciones, importe, ld.fecha_autorizacion,
                            ...
                            FROM flash_ddis_descuentos ld
                            INNER JOIN flash_sucursales_carteros sc ON ld.cartero_id = sc.id";
		...
		$vista_interna = array(
			...
			'results' => $results,
                        ...
                        'acreditaciones' => $this->codegen_model->get('flash_ddis_descuentos','*',''),
                        'autorizadores' => $this->getDdisAutorizadores(),
		);

		$vista_externa = array(			
			'title' => ucwords("Liquidacion Descuentos"),
	...
	function add(){
		...
		if ($this->input->post('enviar_form')){
			...
			$this->codegen_model->add('flash_ddis_descuentos',$data);
		...
		$vista_externa = array(			
			'title' => ucwords("Conceptos de Descuentos"),
			'contenido_main' => $this->load->view('components/liquidaciones/liquidaciones_ddis_descuentos/liquidaciones_ddis_descuentos_add', $vista_interna, true)
		);		  
	...
	function edit($descuento_id = null){
		...
		if ($this->input->post('enviar_form')){
                    ...
                    $descuento_id = $this->input->post('descuento_id_p');
                    if ($valido_desde){
                        $valido_desde = new DateTime($valido_desde);
                    }
                    if($valido_hasta){
			...
			$data = array(
                    ...
                    $this->codegen_model->edit('flash_ddis_descuentos',$data,'id',$descuento_id);
//                    echo $this->db->last_query();die;
                    redirect(base_url().'liquidaciones/liquidaciones_ddis_descuentos');
		}
                
                
                $result = $this->getDescuentos($descuento_id);
//                var_dump($result);die;
		$vista_interna = array(
			'permisos_efectivos' => $this->permisos,
			'result' => $result[0],
...
//	}
	...
	function json($id){
		$json = $this->codegen_model->row('flash_ddis_descuentos','*','id = '.$id);
		if($json) echo json_encode($json);
		else echo json_encode(array('status' => 'none'));
	}

	function json_all(){
		$json = $this->codegen_model->get('flash_ddis_descuentos','*','');
		if($json) echo json_encode($json);
		else echo json_encode(array('status' => 'none'));
	}
	
	function delete($id){
		$this->codegen_model->delete('flash_ddis_descuentos','id',$id);       
	}
        
        function getDescuentos($id = null){
            $where = $id == null?" ":" WHERE la.id = ".$id;
            $query = "  SELECT la.id descuento_id, sc.id cartero_id, sc.apellido_nombre cartero, concepto, 
                                    ld.observaciones, importe, 
                                    date_format(fecha_desde,'%d-%m-%Y') fecha_desde, 
                                    date_format(fecha_hasta,'%d-%m-%Y') fecha_hasta, 
                                    autorizador_id, 
                                    CASE WHEN tipo_concepto = 'G' THEN 'GENERAL'
...
//        function getLiquidacionesDescuentossDetalles($id = null, $acreditacion_id = null){
...
//    }
    ...
    public function autorizarDescuentosDdi($descuento_id){
        
        
            $user_row = $this->ion_auth->user()->row();
            
            $query = " UPDATE flash_ddis_descuentos lad "
                    . "SET  "
