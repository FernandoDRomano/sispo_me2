...
//                        echo $this->db->last_query();die;
                        if ($this->input->post('cartero_id') > 0){
                            $data_cartero = array(
					'ddi_valor_entrega_id' => $this->db->insert_id(),
					'cartero_id' => $this->input->post('cartero_id'),
					'pago_especial' => $this->input->post('valor'),
					'observaciones' => $this->input->post('observaciones'),
					'create' => date('Y-m-d H:m:s'),
					'create_user_id' => $user_row->id,
				);
                            $this->codegen_model->add('flash_ddis_valores_especiales_entregas',$data_cartero);
                        }
//                        echo $this->db->last_query();die;
