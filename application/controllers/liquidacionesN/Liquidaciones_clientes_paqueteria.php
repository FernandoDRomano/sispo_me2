...
class Liquidaciones_clientes_paqueteria extends MY_Controller
{
    ...
    function index()
    {
        ...
        $vista_externa = array(
            ...
            'contenido_main' => $this->load->view('components/liquidacionesN/liquidaciones_clientes_paqueteria_list', $vista_interna, true)
        );
    ...
    public function filtro()
    {
        ...
        $this->view = 'components/liquidacionesN/liquidaciones_clientes_paqueteria_list';
    ...
    function add()
    {
        ...
        if ($this->input->post('enviar_form')) {
            ...
            if ($liquidacion_cliente_id == !NULL) {
                ...
                for ($j = 0; $j < $cantidad_filas; $j++) {
                    ...
                    $liquidacion_comprobantes[$j] = array(
                        ...
                        'servicio' => $datos[6],
                        'cant_total' => $datos[7] ? $datos[7] : 1, //HACE REFERENCIA A LOS BULTOS PERO DEBO PENSAR COMO TRABAJARLO CON LOS DEMAS METODOS, BULTOS, PALETS, METROS CUBICOS
                        'cant_a_liquidar' => 0, //$this->input->post('cantidad_liquidar_'.$j),
                        'precio' => $datos[9],
                        'precio_unitario' => $datos[10],
                        'precio_valor_declarado' => $datos[11],
                        'precio_cobranza' => $datos[12],
                    ...
                    //print_r($liquidacion_comprobantes[$j]);
                }
            ...
            //redirect(base_url() . 'liquidacionesN');
        }
        $vista_interna = array(
            'permisos_efectivos' => $this->permisos,
        ...
        $vista_externa = array(
            'title' => ucwords("Liquidaciones"),
            'contenido_main' => $this->load->view('components/liquidacionesN/liquidaciones_clientes_paqueteria_add', $vista_interna, true)
        );
    ...
    function edit($id)
    {
        ...
        if ($this->input->post('enviar_form')) {                  //  var_dump($post);die;
            ...
            //redirect(base_url() . 'liquidacionesN/liquidaciones_clientes_paqueteria/edit/' . $id);
            redirect(base_url(). 'liquidacionesN/liquidaciones_clientes_paqueteria');
        }
        //Cabecera
        ...
        $query = " SELECT 
                        ci.numero ,
                        cp.*, 
                        DATE_FORMAT(cp.create, '%d-%m-%Y') fecha_liq,
                        pp.*,
        ...
        $vista_externa = array(
            'title' => ucwords("Liquidaciones"),
            'contenido_main' => $this->load->view('components/liquidacionesN/liquidaciones_clientes_paqueteria_edit', $vista_interna, true)
        );
    ...
    function view($id)
    {
        ...
        $vista_externa = array(
            'title' => ucwords("Liquidaciones"),
            'contenido_main' => $this->load->view('components/liquidacionesN/liquidaciones_clientes_paqueteria_view', $vista_interna, true)
        );
    ...
    function anular($liquidacion_cliente_id)
    {
        ...
        foreach ($liquidaciones_comprobantes as $value) {
            ...
            */
            ...
            $query_back_ci = "UPDATE  flash_comprobantes_ingresos ci 
                              SET ci.estado_liquidacion = 13, /* CODIGO 13 = ANULADO, NUEVO CODIGO QUE ME DIJO ALVARO QUE LE PONGA A LA HORA DE ANULAR LA FACTURA */
                                  ci.liquidacion_id = 0
                              WHERE ci.id = " . $value->ci_id;
    ...
    function exportarAPdf()
    {
            ...
            $html_encabezado = '<table class="table table-striped table-hover table-condensed bootstrap-datatable table-bordered" border="0">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="border-bottom:.5px solid #000000;text-align:center">Palets</th>
                                            <th style="border-bottom:.5px solid #000000;text-align:center">Metros Cubicos</th>
            ...
            foreach ($detalles as $value) {
                ...
                if ($value->liquidacion_ajuste_id == 0) {
                    $html_cuerpo    .= '<tr>
                                            ...
                                            <td style="text-align:right;">' . $value->palets . '</td>
                                            <td style="text-align:right;">' . $value->metro_cubico . '</td>
        ...
        $pdf->SetHeaderData(
            ...
            array(0, 64, 128),
            $liquidacion_nro,
            Cliente::whereId($cliente_id)->first()->nombre,
            $factura_nro,
            $periodo_desde,
            $periodo_hasta,
            $sucursal,
