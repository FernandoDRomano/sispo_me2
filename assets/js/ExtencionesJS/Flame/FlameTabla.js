...
//Establece El Paginador Desde El Elemento
	...
	function FormatearDatosParaTabla(Config = false){
		...
		//console.log(Config.MensajeEnNulo);
		//console.log(Config.TextoEnNulo);
		if(! (Config.Resultado[0].length>0)){
			if(Config.MensajeEnNulo){
				DivDeTabla.setAttribute("hidden", "");
				FlameGrowlFail(Config.TextoEnNulo);
			}
		}else{
			DivDeTabla.removeAttribute("hidden");
		}
		
		if(Config.Resultado[0].length>0){
			//console.log(Config.Resultado[0].length);
			if(Config.AddCheckbox != null){
				var PreDivFullCheckbox = document.getElementsByClassName("DivFullCheckbox");
				for(var i =0;i<PreDivFullCheckbox.length;i++){
					if( !(typeof(PreDivFullCheckbox[i]) == 'undefined') ){
						var PadreDiv = PreDivFullCheckbox[i];
						//console.log(PadreDiv);
						//console.log(PadreDiv.id);
						while(typeof(PadreDiv) != 'undefined' && PadreDiv != null){
							//console.log(PadreDiv.id);
							//console.log(Config.DivContenedor);
							if(PadreDiv.id == Config.DivContenedor){
								PreDivFullCheckbox[i].remove();
							}
							PadreDiv = PadreDiv.parentElement;
						}
					}
				}
	...
	function CrearTabla(Config){
		...
		*/
		...
		for(var i = 0 ; i <Config.Titulos.length ; i++ ){
			...
			if(Config.BotonsParaTabla != null && i+1 == Config.Titulos.length){
				for(var Botoni = 0; Botoni < Config.BotonsParaTabla.length; Botoni++){
					//console.log(Config.BotonsParaTabla[Botoni]);
					var Columna = document.createElement("TH");
					Columna.setAttribute("id", "Titulos_TH"+i+"-Boton");
					Columna.Numero = i;
					var Texto = document.createElement("P");
					Texto.innerHTML = Config.BotonsParaTabla[Botoni].Columna;
					Columna.appendChild(Texto);
					Titulos.appendChild(Columna);
					/*
					for(var Botonj = 0; Botonj < Config.Funciones.length; Botonj++){
				...
				for(var j = 0; j < Config.Resultado[i].length;j++){
					...
					if(Config.EsconderElementos != undefined){
						if(Config.EsconderElementos.find(element => element === j) != undefined){
							...
							Columna.style.display="none";
							var Texto;
							if(Config.InterpreteHTML != undefined){
								if(Config.InterpreteHTML){
									//console.log("if");
									Texto = document.createElement("DIV");
									Texto.innerHTML = Config.Resultado[i][j];
								}else{
									//console.log("else1");
									Texto = document.createTextNode(Config.Resultado[i][j]);
					...
					//if(Config.BotonsParaTabla != null && i+1 == Config.Titulos.length){
					if(Config.BotonsParaTabla != null && j+1 == Config.Resultado[i].length){
						for(var Botoni = 0; Botoni < Config.BotonsParaTabla.length; Botoni++){
							var Columna = document.createElement("TD");
							...
							for(var Botonj = 0; Botonj < Config.BotonsParaTabla[Botoni].Funciones.length; Botonj++){
								var Boton = document.createElement("BUTTON");
								Boton.className = Config.BotonsParaTabla[Botoni].Classe[Botonj];
								var TextoDeBoton = document.createTextNode("" + Config.BotonsParaTabla[Botoni].Texto[Botonj]);
								Boton.id = "Boton-" + Botoni;
								Boton.Data = i;
								Boton.DataX = Botoni;
								Boton.DataY = Botonj;
								Boton.onclick = function(){
									...
									//var DivDeTabla = this.parentElement.parentElement.parentElement.parentElement;
									eval(Config.BotonsParaTabla[this.DataX].Funciones[this.DataY])(this);
								};
								var Icono = document.createElement("I");
								Icono.className= "" + Config.BotonsParaTabla[Botoni].Icono[Botonj];
								Icono.style="" + Config.BotonsParaTabla[Botoni].EstiloDeIcono[Botonj];
						...
						if(filaconclick){
							fila.onclick = function(e){
								if(e.target !== this){
									...
									if(typeof e.target.onclick === "function" || e.target.tagName == 'BUTTON'|| e.target.parentElement.tagName == 'BUTTON' || e.target.tagName == 'A'|| e.target.parentElement.tagName == 'A'){
										console.log(e.target);
										return;
									}else{
										console.log(e.target);
		...
		//var DivDeTabla =  document.getElementById(Config.DivContenedor);
		var ElementosDeTabla = tabla.getElementsByTagName("tr");
		var PaginadorLineas = DivDeTabla.getElementsByClassName('PaginadorLineas')[0].getElementsByTagName('p')[0];
		if( Config.Resultado.length > 0){
			if(Config.ConFiltro){
				PaginadorLineas.innerHTML='Mostrando ' + (ElementosDeTabla.length-2) + ' de ' + Config.Resultado.length + ' Resultados';
			}else{
				PaginadorLineas.innerHTML='Mostrando ' + (ElementosDeTabla.length-1) + ' de ' + Config.Resultado.length + ' Resultados';
			}
		}else{
			PaginadorLineas.innerHTML='';
		}
		//PaginadorLineas.innerHTML='Mostrando ' + (ElementosDeTabla.length-2) + ' de ' + Config.Resultado.length + ' Resultados';
		//console.log(DivDeTabla.Config);
						...
						*/
						...
						if(Config.InterpreteHTML != undefined){
							if(Config.InterpreteHTML){
								//console.log("if");
								Texto = document.createElement("DIV");
								Texto.innerHTML = Config.ResultadoFiltrado[i][j];
							}else{
								//console.log("else1");
								Texto = document.createTextNode(Config.ResultadoFiltrado[i][j]);
							}
						}else{
							//console.log("else2");
								Texto = document.createTextNode(Config.ResultadoFiltrado[i][j]);
						...
						if(Config.BotonsParaTabla != null && j+1 == Config.Resultado[i].length){
							for(var Botoni = 0; Botoni < Config.BotonsParaTabla.length; Botoni++){
								...
								for(var Botonj = 0; Botonj < Config.BotonsParaTabla[Botoni].Funciones.length; Botonj++){
									...
									Boton.className = Config.BotonsParaTabla[Botoni].Classe[Botonj];
									var TextoDeBoton = document.createTextNode("" + Config.BotonsParaTabla[Botoni].Texto[Botonj]);
									Boton.id = "Boton-" + Config.FilasResultadosDeFiltros[i] + "-" + Botoni + "-" + Botonj;
