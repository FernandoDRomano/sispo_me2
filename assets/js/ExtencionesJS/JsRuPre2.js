	...
	function OnResultChangeCompleteSelect(ArraydChangeComplete,Elementos){
		...
		if(ArraydRetorno!=false && ArraydRetorno!=undefined){
			...
			NotSussesDeleteSelect(Elementos[0]);
			for(var i=1;i<Elementos.length;i++){
	...
	function ChangeCompleteSelect(ElementId,ElementTexto,TextoInicial,Ajax,Elementos,ElementosTextos){
		...
		$("#"+ElementId).change(function() {
			...
			if (this.value == 0) {
				...
				OnResultChangeCompleteSelect(ArraydChangeComplete,Elementos);
				ElementTexto.innerHTML = "<b style='color:#FF0000;font-size: 10px;width: 90%;'>"+TextoInicial+"</b>";
				EndLoading();
			}
			else {
				...
				AjaxArraydSelectAsync(filtro,filtroX,Ajax,Elementos);
				//ArraydRetorno = ArraydChangeComplete;
			}
			var paragrap = document.getElementById("Paragrap");
	...
	function AjaxArraydSelectAsync(DataNombre,Data,Ajax,Elementos){
		Loading();
		var paragrap = document.getElementById("Paragrap");
		var xhttp;
		xhttp = new XMLHttpRequest();
